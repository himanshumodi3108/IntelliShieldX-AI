...
const Profile = () => {
  ...
  const handleToggleMFA = async (enabled: boolean) => {
    try {
      ...
      if (enabled) {
        ...
        await userApi.enableMFA(mfaMethod);
  ...
  const handleSetupSMSOTP = async () => {
    ...
    try {
      setIsLoading(true);
      await userApi.setupSMSOTP(phoneNumber);
      setMfaMethod("sms");
  ...
  const handleSetupTOTP = async () => {
    try {
      setIsLoading(true);
      const result = await userApi.setupTOTP();
  ...
  const handleVerifyTOTP = async () => {
    ...
    try {
      setIsLoading(true);
      await userApi.verifyTOTP(totpCode);
      ...
      <main className="pt-24 pb-16">
        <div className="container px-4 max-w-6xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Manage your account settings, connected accounts, and repositories</p>
          </div>
          ...
          <Tabs defaultValue="profile" className="space-y-6">
            <TabsList className="grid w-full grid-cols-4 bg-secondary/50 p-1 rounded-xl">
              ...
              <TabsTrigger value="repositories" className="rounded-lg">
                Repositories
              </TabsTrigger>
            </TabsList>
